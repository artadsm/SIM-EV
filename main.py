...
def simulateCharging(ev,cu,startTime,endTime,interval):
    time = startTime
    chargingPower = min(ev.maxPower,cu.maxPower)
    data = [[time,chargingPower,ev.stateOfCharge,0]]
    i = 0

    while time < endTime:
        if chargingPower*(interval.total_seconds()/3600)*(i+2) > ev.batteryCapacity:
...
df = simulateCharging(ev,cu,startTime,endTime,interval)
